...
const DetailsPage = () => {
  ...
  const { name, imageUrl, type, description, price, rating } = useLocalSearchParams() as { name: string, imageUrl: string, type: string, description: string, price: string, rating: string };
  ...
  return (
    <GestureHandlerRootView
      className='bg-[#F9F9F9] w-full h-full m-1'
    >
      ...
      <View className='h-full flex-col justify-between'>
        <ScrollView>
            <View className='mx-5 items-center'>
              <DetailsHeader image_url={imageUrl} name={name} type={type} rating={Number(rating)} />
        ...
        > 
          ...
          <TouchableOpacity 
                ...
                onPress = {buyNow}
              >
                <Text className="text-xl color-white font-[Sora-Regular]">Add to Cart</Text> 
          </TouchableOpacity> 
