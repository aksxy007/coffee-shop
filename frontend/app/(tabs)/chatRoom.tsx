...
const ChatRoom = () => {
  ...
  const getChatBotResponse = async (message:any)=>{
    try {
      const response = await axios.post("http://192.168.29.13:8000/chat/get-response",{
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (error.response && error.response.status === 401) {
          ...
          console.error("Error fetching agent response:", error.response?.data || error.message);
        }
      } else {
    ...
    try {
      if (!token) {
        ...
        if(inputRef) inputRef?.current?.clear();
          ...
          let resposnseMessage = await getChatBotResponse(message);
