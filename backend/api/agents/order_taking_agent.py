    ...
    def get_response(self,messages):
        ...
        system_prompt = """
            ...
            * Don't tell the user to go to a place to get the order
            ...
            4. if the user did not confirm with a specific item but a general item like syrup not specifiying which one confirm with the user by listing the items when have of that type.(example user says syrup , ask which one Chocolate syrup - $1.50
                Hazelnut syrup - $1.50
                Carmel syrup - $1.50
                Sugar Free Vanilla syrup - $1.50 )
            5. Ask them if they need anything else.
            6. If they do then repeat starting from step 3
            7. If they don't want anything else stating thanks or anything,Using the "order" object that is in the output, make sure to hit all three points
        ...
        chatbot_output = get_chatbot_response(model_name=self.model_name,messages=input_messages)
